...
void heap::print() {
  cout << "(" << huffTree[0]->freq << ") ";
  for ( int i = 1; i <= heap_size; i++ ) {
    cout << huffTree[i]->letter << huffTree[i]->freq << " ";
...
unordered_map<char,string> heap::pathname(huffmanNode* node, string p) {
  if (node->left != NULL && node->right != NULL) {
    pathname(node->left, p + "0");
    pathname(node->right, p + "1");
  }

  if (node->letter == ' ') {
    cout << "space" << " " << p << endl;
    prefixCodes[node->letter] = p;
  }
  if (node->left == NULL && node->right == NULL && node->letter != ' ') {
    cout << node->letter << " " << p << endl;
    prefixCodes[node->letter] = p;
  }
