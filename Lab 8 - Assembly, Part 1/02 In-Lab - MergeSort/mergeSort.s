...
start:
	...
	mov	rcx, rsi	; mid = (start + end) / 2
	add	rcx, rdx
	shr	rcx, 1
	push	rcx		; push "mid" onto stack
	push 	rdx		; push "end" onto stack
	
	mov	rdx, rcx	; make "mid" the end
	call 	mergeSort	; MERGESORT (A[ ], start, mid)  		; _mergeSort
	
	pop 	rdx		; pop "end" into rdx
	pop	rcx		; pop "mid" into rcx
	push	rcx		; push "mid" onto stack
	push 	rsi		; push "start" onto stack
	push	rdx		; push "end" onto stack
	
	add 	rcx, 1		; mid+1
	mov	rsi, rcx	; make "mid"+1 the start
	call	mergeSort	; MERGESORT (A[ ], mid+1, end)			; _mergeSort
	
	pop 	rcx		; pop "end" into end (4th argument)
	pop	rsi		; pop "start" into start (2nd argument)
	pop	rdx		; pop "mid" into mid (3rd argument)
	
	push	r10		; save
	push 	r11		; save
	call 	merge		; MERGE (A[ ], start, mid, end)			; _merge
	pop	r11		; restore
	pop	r10		; restore
